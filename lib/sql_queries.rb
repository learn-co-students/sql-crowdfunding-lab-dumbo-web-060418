...
# "SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY SUM(pledges.amount)"
"SELECT users.name AS userName, SUM(pledges.amount) AS pledgeTotal FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledgeTotal"
end
