...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT users.name, users.age, SUM(pledges.amount) FROM users
    INNER JOIN pledges ON users.id = pledges.user_id GROUP BY pledges.user_id ORDER BY users.name"
end
...
#expected answer: [["My book on SQL", 0], ["The next Inna-Gadda-Davida", 142]]
#difference btwn pledge amount and goal: SUM(pledges.amount) - projects.funding_goal
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) FROM users INNER JOIN pledges on users.id = pledges.user_id
...
"SELECT projects.category, SUM(pledges.amount) FROM projects INNER JOIN pledges on projects.id = pledges.project_id
